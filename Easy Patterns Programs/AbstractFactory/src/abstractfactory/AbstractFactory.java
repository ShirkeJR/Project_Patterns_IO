...
class AnimalAbstractFactory{
    public SpeciesFactory getFactory(String type){
        if(type.equals("Cat")) return new CatFactory();
...
class CatFactory extends SpeciesFactory{
    ...
    public Animal getAnimal(String name) {
        if(name.equals("Stefan")) return new Cat(name);
        else return new Cat(name);
    }
}
class DogFactory extends SpeciesFactory{
    @Override
    public Animal getAnimal(String name) {
       if(name.equals("Stefan")) return new Dog(name);
       else return new Dog(name);
    }
}
